...
app.openapi(
  createRoute({
    ...
    responses: {
      200: {
        content: {
          "application/json": {
            schema: z.object({ success: z.literal(true), data: SurahWithVersesSchema }),
          },
        },
  ...
  async (c) => {
    ...
    const { sourceId: sourceIdParam } = c.req.valid("query");
    ...
    try {
      ...
      // Fetch Arabic verse text from quran_translations
      ...
      // Fetch footnotes for all returned verse translations in a single query,
      // then group them by verse_translation_id for O(1) lookup below.
      ...
      return c.json(
        {
          success: true as const,
          data: { ...surah, sourceId, verses } as any,
        },
        200
      );
    } catch (e) {
